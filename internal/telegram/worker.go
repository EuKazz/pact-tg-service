...
// структура-обертка для объекдинения авторизации через куар с механизмом обратного вызова
type QRAuthWrapper struct {
	*qrlogin.QR
	OnCode func(ctx context.Context, token qrlogin.Token) error
}

// метод для запуска QR
...
func (w *Worker) Start() {
	go func() {
		// гарантия закрытия канала при выходе из горутины
		...
		err := w.client.Run(w.ctx, func(ctx context.Context) error {
			// проверка статуса: если уже авторизованы (через файл), идем дальше
			status, _ := w.client.Auth().Status(ctx)
			if status.Authorized {
				w.logger.Info().Msg("Сессия уже активна, пропускаем вход")
				// закрытие канала, чтобы менеджер не ждал QR
				select {
				case <-w.QRCodeChan:
				default:
				}
				...
				// вызов метода для аутент через куар-код
			...
			// блокировка до вызова Stop()
			<-ctx.Done()
			return ctx.Err()
		})
		// логирование причину остановки
